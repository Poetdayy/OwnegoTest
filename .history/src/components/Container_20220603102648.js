...
const Container = (props) => {
  ...
  useEffect(() => {
    const checkProducts = shopProducts.filter((item) => item.shop === 1);
    setproductsInStore(checkProducts);
  }, []);
  console.log(productsInStore);

  const getCurrent = useEffect(() => {
    function getItem() {
      const productsInCurrentStore = [];
      for (let i = 0; i < Products.length; i++) {
        for (let j = 0; j < productsInStore.length; j++) {
          if (productsInStore[j].product === Products[i].id)
            productsInCurrentStore.push(Products[i]);
        }
      }
      return setProductsCurrent(productsInCurrentStore);
    }
  }, []);

  function handleSortItem(sortState) {
    if (sortState === "one") {
    ...
    <div className="content">
      <div className="content_container">
        ...
        <div className="content_button">
          <button className="store_filter_buttonbtn btn-primary">Filter</button>
        ...
        <div className="listItem">
          {handleSortItem(sortState)
            ? handleSortItem(sortState).map((item) => {
                return (
                  <Card
                    key={item.id}
                    id={item.id}
